...
bootstrap_samples = tibble(
  strap_number = 1:5000,
  strap_sample = map(1:5000, \(i) sample_frac(weather_df, replace = TRUE))
)
```
...
bootstrap_results = 
  bootstrap_samples %>%
  mutate(
    model = map(strap_sample, \(df) lm(tmax ~ tmin, data = df)),
    r_squared = map_dbl(model, \(mod) broom::glance(mod)$r.squared),
    log_product = map_dbl(model, \(mod) {
...
Plot the distributions of r squared and log betas
